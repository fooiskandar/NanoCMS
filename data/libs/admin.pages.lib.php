...
function performMove() {
	if( $_GET[action]=='reorder' ) {
		...
		header( "Location:admin.php?action=showpages" );
	}
}
	...
	<form action='$formAction' method=post class='pagemod_form' accept-charset='utf-8'>
		...
		<table width='98%' border='0'>
		  <tr>
			...
			<td><input type='text' size=50 value='$title' name='title'></td>
...
//		echo "<h2 class='cattitle noborder' id='h2$v'><a href='#nogo'><span id='co$v' class='togg'>&raquo;</span> $catname</a> $removeOpt</h2>";
		...
		foreach($slugids as $pos=>$ids) {
			...
			$ul = makeLink("?action=reorder&cat=$catname&param=".$pos.','.($pos-1),"<img src='".NANO_ADMIND_DESIGN_BASE."icons/arrow_up.png' alt='$upTxt' title='$upTxt'/>");
			$dl = makeLink("?action=reorder&cat=$catname&param=".$pos.','.($pos+1),"<img src='".NANO_ADMIND_DESIGN_BASE."icons/arrow_down.png' alt='$downTxt' title='$downTxt' />");

			if( $pos==0 ) $ul="<img src='".NANO_ADMIND_DESIGN_BASE."icons/arrow_up_d.png' alt='$upTxt' />";
			if( $pos==$n ) $dl="<img src='".NANO_ADMIND_DESIGN_BASE."icons/arrow_down_d.png' alt='$downTxt' />";
			...
			$s = "<tr>
					...
					<td class='center' width='10px'><a href='?action=edit&slug=".$slugs[$ids]."'><img src='".NANO_ADMIND_DESIGN_BASE."icons/page_edit.png' alt='$editTxt' title='$editTxt' /></a></td>
					<td class='center' width='10px'><a href='?action=delete&slug=".$slugs[$ids]."' onclick='return confirm(\"$deleteConfirmMsg\");'><img src='".NANO_ADMIND_DESIGN_BASE."icons/cross.png' alt='$delTxt' title='$delTxt' /></a>
					</td>
